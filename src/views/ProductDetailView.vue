    ...
    <div v-if="product" class="row g-5">
      ...
      <div class="col-lg-7">
        ...
        <div class="bg-light p-4 rounded-3 mb-4">
          <p class="h2 text-danger fw-bold mb-0">
            {{ formatPrice(product.price) }}đ
          </p>
          <del class="text-muted ms-3" v-if="product.originalPrice">
        ...
        <div class="d-flex gap-3">
          <button @click="addToCart" class="btn btn-danger btn-lg px-5 py-3 rounded-pill shadow">
            <i class="fas fa-cart-plus me-2"></i>Thêm vào giỏ hàng
          </button>
          <button class="btn btn-outline-danger btn-lg px-5 py-3 rounded-pill">
        ...
        <div class="row text-muted">
          ...
          <div class="col-sm-6">
            <strong>Giao hàng:</strong> Toàn quốc – Freeship đơn từ 300k
          </div>
          <div class="col-sm-6">
            <strong>Bảo hành:</strong> Đổi trả trong 7 ngày
          </div>
        </div>
      </div>
    </div>
...
onMounted(() => {
  ...
  product.value = products.find(p => p.id === id) || products[0]
})
